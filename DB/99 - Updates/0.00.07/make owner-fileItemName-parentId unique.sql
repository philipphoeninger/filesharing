...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20241029213105_UpdateItemsAndLinksOwner'
)
BEGIN
    EXEC sp_rename N'[dbo].[Links].[UserId]', N'Owner', N'COLUMN';
END;
GO
...
BEGIN
    EXEC sp_rename N'[dbo].[Links].[IX_Links_UserId]', N'IX_Links_Owner', N'INDEX';
END;
GO
...
BEGIN
    ALTER TABLE [dbo].[FileItems] ADD [Owner] nvarchar(200) NOT NULL DEFAULT N'';
END;
GO
...
BEGIN
    CREATE INDEX [IX_FileItems_Owner] ON [dbo].[FileItems] ([Owner]);
END;
GO
...
BEGIN
    ALTER TABLE [dbo].[FileItems] ADD CONSTRAINT [FK_FileItems_User] FOREIGN KEY ([Owner]) REFERENCES [identity].[Users] ([Id]) ON DELETE NO ACTION;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20241029213105_UpdateItemsAndLinksOwner', N'8.0.8');
END;
GO
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20241029214331_MakeOwnerFileItemNameParentIdUnique'
)
BEGIN
    DROP INDEX [IX_FileItems_Owner] ON [dbo].[FileItems];
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_User_FileItems_Folder] ON [dbo].[FileItems] ([Owner], [Name], [ParentId]) WHERE [ParentId] IS NOT NULL');
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20241029214331_MakeOwnerFileItemNameParentIdUnique', N'8.0.8');
END;
GO
